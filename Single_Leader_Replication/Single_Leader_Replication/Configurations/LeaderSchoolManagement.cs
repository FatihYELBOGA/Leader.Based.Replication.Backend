...
{
    public class LeaderSchoolManagement : DbContext
    {
        public LeaderSchoolManagement(DbContextOptions<LeaderSchoolManagement> options) : base(options) { }
        ...
        public static void Seed(LeaderSchoolManagement leaderDatabase)
        {
            if (leaderDatabase.Database.GetPendingMigrations().Count() == 0)
            {
                if (leaderDatabase.Students.Count() == 0)
                {
                    leaderDatabase.Departments.AddRange(departments);
                    leaderDatabase.Students.AddRange(students);
                    leaderDatabase.Courses.AddRange(courses);
                    leaderDatabase.StudentCourses.AddRange(studentCourses);

                    leaderDatabase.SaveChanges();
                }
            }
        }
